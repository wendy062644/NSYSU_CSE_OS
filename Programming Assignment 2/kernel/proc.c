...
//#include <minix/sys/sys/envsys.h>
...
int process_sum = 0, MAX_TICKETS = 10, MIN_TICKETS = 1;
...
void timer_interrupt_handler() {
    ...
    if (timer_interrupt_count % 100 == 0) {
        timer_interrupt_count = 0;
        printf("%.2f\n", (float)process_sum / 100.0);
    }
}

void ProcessSetPriority(struct proc *p, int n) {
    p -> p_tickets += n;
}
	...
	if (proc_is_preempted(p)) {
		...
		if (proc_is_runnable(p)) {
			if (p->p_cpu_time_left) {
				if (p -> p_tickets < MAX_TICKETS)
			        	ProcessSetPriority(p, 1);
				enqueue_head(p);
			}
			else
				enqueue(p);
 ...
 */
  ...
  lottery = 1 + random() % total_tickets;
  total_tickets = 0;
...
void proc_no_time(struct proc * p)
{
	if (!proc_kernel_scheduler(p) && priv(p)->s_flags & PREEMPTIBLE) {
		if(p -> p_tickets > MAX_TICKETS) { // 當Process完成前沒有被Block
		    p -> p_priority++;
	            ProcessSetPriority(p, -1);  // 且ticket大於2時，會少一張
	        }
